...
// this code will go into the esp26-devkitm-1 that will control the AC-DC Voltage Covertor
...
void VoltDis()
{
  ...
  if (Timer3 == 5) {
    if (CVolt != textv && CVolt < 10 || Startv == 0) {
      lcd.setCursor(1, 1);
      lcd.print(CVolt);
      textv = CVolt;
      Startv = 1;
...
void Currentmeasure()
{
  ...
  // For this test code the CVolt value will show where the Voltage Value would go on the Display
  CVolt = Current; // CVolt show what the Voltage is at current reader
  Current = (Current - (2.37)) / 0.044;
  // Once you get the CVolt value at a stable place replace the value in the brackets above to 
  // set a new zero point for current reader
...
//  }
  ledcWrite(ledPin, dutyCycle);
// For testing the current or just leaving the current on the Current control Program is disabled
}
//--------------------------------------Status----------------------------------------------//
void Status()
{
